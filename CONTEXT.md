...
### Boxes Per Pallet Normalization ("ตามความเหมาะสม")
A specialized data extraction and normalization rule in `normalizers.ts` for the `boxes_per_pallet` field in Master Sheets and Drawings. If a document specifies numeric values (e.g. 24), the digits are extracted cleanly. If the document states "ตามความเหมาะสม" or keyword variations of "ความเหมาะสม" / "appropriate", both the Gemini OCR prompt and the normalizer return "ตามความเหมาะสม" directly, avoiding artificial numeric hallucination.
