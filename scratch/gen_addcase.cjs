                ...
                <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6 bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
                  <div className="col-span-2 sm:col-span-1">
                    <Controller
                      ...
                      render={({ field }) => (
                        <InputField label="หมายเลขล็อต (Batch no.)" type="date" value={convertDMYToYMD(field.value || '')} onChange={(e) => field.onChange(convertYMDToDMY(e.target.value))} disabled={isSaving} />
                      )}
                    />
                  </div>
                  <div className="col-span-2 sm:col-span-1">
                    <Controller
                      ...
                      render={({ field }) => (
                        <InputField label="วันที่ผลิตแกลลอน" type="date" value={convertDMYToYMD(field.value || '')} onChange={(e) => field.onChange(convertYMDToDMY(e.target.value))} disabled={isSaving} />
                      )}
                    />
                  </div>
...
console.log('AddCaseTab rewritten.');
`;
