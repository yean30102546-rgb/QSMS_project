...
* **Backend API Gateway:** พัฒนาด้วย Next.js Route Handlers ทำหน้าที่เป็นด่านความปลอดภัยหน้าด่าน (API Security Boundary) เพื่อซ่อนข้อมูล API Key และเชื่อมต่อ Supabase DB
...
เพื่อความปลอดภัยในระดับอุตสาหกรรม ระบบเลือกใช้โครงสร้าง **API Gateway Pattern** (Richardson, 2018) ผ่านระบบ Next.js API Route Handlers หน้าด่าน ตัวเซิร์ฟเวอร์จะคัดกรองสิทธิ์และคีย์สิทธิ์ของผู้ใช้เพื่อเชื่อมต่อไปยัง Supabase ด้านหลัง ป้องกันการโจมตีประเภท Server-Side Request Forgery (SSRF) และซ่อนข้อมูล Credentials ทั้งหมดไม่ให้รั่วไหลไปสู่ฝั่งไคลเอนต์
...
| `status` | VARCHAR | สถานะเคส (`Pending`, `In-Progress`, `Completed`) |
