...
## BUG-015: Duplicate Item Number + Reason Combinations Blocked at Backend Without Frontend Validation
**Status**: ✅ FIXED (2026-05-27)
- *Problem*: เมื่อผู้ใช้งานกรอกรายการสินค้าที่มีข้อมูลซ้ำซ้อนกันในฟอร์มเดียวกัน (มีบาร์โค้ด, สาเหตุ, เลขกล่อง, Mold, และ Line เหมือนกันทุกประการ) ปุ่มบันทึกยังคงเปิดให้กดได้ เมื่อกดบันทึกแล้วระบบจะส่งไปตรวจสอบที่ backend และถูกตีกลับด้วยข้อความ Error `Duplicate Item Number + Reason combinations found...` ทำให้ผู้ใช้เกิดความสับสนและคิดว่าระบบขัดข้อง
- *Solution*: 
  1. อัปเดตตัวตรวจสอบความถูกต้องบน Frontend (`isSaveDisabled` ใน `src/services/validation.ts`) ให้เรียกใช้งาน `findDuplicateItemNumbers` เพื่อตรวจสอบความซ้ำซ้อนของข้อมูลก่อนเสมอ และปิดปุ่มบันทึกหากพบว่ามีรายการซ้ำกันในฟอร์ม (รวมถึงอนุญาตให้บันทึกได้หาก `reasonSubtype` แตกต่างกัน โดยรวม `reasonSubtype` เข้าใน Composite Key)
  2. เพิ่มแถบแจ้งเตือนสีแดง (Error Warning Banner) ด้านบนปุ่มบันทึกในหน้า `AddCaseTab.tsx` เพื่อแสดงรายการที่ซ้ำซ้อนพร้อมชี้แจงวิธีการแก้ไข (แก้ไขหรือกรอกเลขกล่อง, Mold, หรือ Line หรือเลือก Subtype ให้แตกต่างกัน) ทำให้ UX มีความสมบูรณ์และชัดเจนมากขึ้น
  3. ปรับปรุงการตรวจสอบในฝั่ง Backend GAS (`gas/Code.gs` -> `checkDuplicateItemNumbers`) ให้ใช้เงื่อนไขคีย์เดียวกันเพื่อให้การทำงานตรงกัน 100%
...
- Ingested Raw Source: [[1_raw/BUG_FIX_VERIFICATION_502953522.md]]
- Ingested Raw Source: [[1_raw/CHANGES_LOG_2021446418.md]]
- Ingested Raw Source: [[1_raw/COMPLETION_REPORT_1974657342.md]]
- Ingested Raw Source: [[1_raw/FINAL_SUMMARY_726519578.md]]
- Ingested Raw Source: [[1_raw/IMPLEMENTATION_SUMMARY_1251533505.md]]
- Ingested Raw Source: [[1_raw/PROJECT_COMPLETION_STATUS_1499868418.md]]
- Ingested Raw Source: [[1_raw/QUICK_FIX_ITEMMASTER_254796264.md]]
- Ingested Raw Source: [[1_raw/TECHNICAL_SUMMARY_1875189856.md]]
- Ingested Raw Source: [[1_raw/VERIFICATION_BUTTON_IMPLEMENTATION_1242890547.md]]
