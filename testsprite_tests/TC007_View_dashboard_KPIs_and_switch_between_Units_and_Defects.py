...
async def run_test():
    ...
    try:
        ...
        # -> Fill the username and password fields, then submit the login form to reach the dashboard.
        ...
        # -> Click the 'แดชบอร์ด (Dashboard)' navigation item to open the Dashboard view, then locate and switch the view to 'Defects' and verify chart analytics are displayed.
        # button "แดชบอร์ด (Dashboard)"
        elem = page.locator("xpath=/html/body/div/div/aside/div[2]/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the 'ความถี่ปัญหา (Defects)' button (interactive element index 738) to switch the view, wait for the UI to update, and verify chart-based analytics are displayed.
